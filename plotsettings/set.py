...
class Set:
    ...
    def __init__(self, journal_name, module="plotsettings.journals"):
        ...
        self.journals = importlib.import_module(
            module
        ).journals  # python module containing a single dict
        params = copy.deepcopy(
            self.journals[journal_name]
        )  # Find the journal 'journal_name'
        # Set rcParams:
        pyplot.rcParams.update(params["rcParams"])  # Change matplotlib rcparams
        # Get figsize variables
        self.size_dict = params["figsize"]
        self.size_dict["column_width"] = self._inches(
            self.size_dict["column_width"], self.size_dict["units"]
        )  # Convert to inches
        self.size_dict["gutter_width"] = self._inches(
            self.size_dict["gutter_width"], self.size_dict["units"]
        )  # Convert to inches
        self.size_dict["max_height"] = self._inches(
            self.size_dict.get("max_height", float("inf")), self.size_dict["units"]
        )  # Convert to inches
        # Get panel label variables
        self.label_dict = params["panel_labels"]
        self.label_dict["fontsize"] = self.label_dict.get(
            "fontsize", params["rcParams"]["font.size"]
        )
    ...
    def set_figsize(self, n_columns, n_rows=1, aspect_ratio=0.618):
        ...
        >>> print '%.3f, %.3f' %(height, width) # Check the figure dimensions
        7.480, 1.772
        >>> publishable.set_figsize(2, 3, 1) # doctest: +IGNORE_EXCEPTION_DETAIL
        ...
        if figure_height > self.size_dict["max_height"]:
            warnings.warn(
                "Specified figure height exceeds maximum height. Setting to maximum height instead"
            )
    ...
    def _calc_width(self, n_columns):
        ...
        """
        return (
            n_columns * self.size_dict["column_width"]
            + (n_columns - 1) * self.size_dict["gutter_width"]
        )
...
def label_generator(case="lower", prefix="", suffix="."):
    ...
    label_generator = (
        "%s%s%s" % (prefix, letter, suffix) for letter in choose_type[case]
    )
...
def label_offsetter(ax, xoffset_axis):
    ...
    x_origin, y_origin = ax.transAxes.transform(
        (0, 0)
    )  # Get the display coordinates at the axis origin
    x_display = ax.transAxes.transform((xoffset_axis, y_origin))[
        0
    ]  # offset in display coordinates (pixels)
    offset = x_display - x_origin  # offset from axis origin in display coordinates
    xoffset_axis, yoffset_axis = ax.transAxes.inverted().transform(
        (offset + x_origin, offset + y_origin)
    )  # Unify offsets
...
def panel_labels(
    ...
    choose_position = {
        "outside": {"va": "bottom", "ha": "right"},
        "inside": {"va": "top", "ha": "left"},
    }
    create_offset = {
        "outside": (lambda x, y: (-x, 1 + y)),
    ...
    if (
        len(xy) == 1
    ):  # If xy has one value, both offsets according to offset in display units of xoffset
        x_offset = xy[0]
        equaloffsets = True
    if len(xy) == 2:  # If both x and y offsets are given
        ...
        equaloffsets = False
    choose_colorbardetector = {
        True: is_colorbar,
        False: lambda ax: False,
    }  # Whether or not to autodetect colorbars
    colorbar_detector = choose_colorbardetector[detect_colorbars]
    for ax in axis_list:
        if (
            colorbar_detector(ax) == False
        ):  # Don't label axes that contain only colorbars
            label = generate_labels.next()  # Get the next label
            if equaloffsets:
                x_axis, y_axis = label_offsetter(
                    ax, x_offset
                )  # Make x and y offsets the same in display coordinates
                x, y = offsetter(
                    x_axis, y_axis
                )  # Turn offsets into positions in axis coordinates
            # Draw label:
            ax.text(
                x,
                y,
...
def dummy_figure(color="Coral"):
    ...
    for ax, title, y, ylabel, xticklabel, xlabel in zip(
        axes, titles, ys, ylabels, xticklabels, xlabels
    ):
